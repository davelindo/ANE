...
NSString *genMIL(int ch, int sp, int depth) {
  ...
  [m appendFormat:@"    func main<ios18>(tensor<fp32, [1, %d, 1, %d]> x) {\n",
                  ch, sp];
  [m appendString:
          @"            string c_pad_type_0 = const()[name = "
          @"string(\"c_pad_type_0\"), val = string(\"valid\")];\n"
          @"            tensor<int32, [2]> c_strides_0 = const()[name = "
          @"string(\"c_strides_0\"), val = tensor<int32, [2]>([1, 1])];\n"
          @"            tensor<int32, [4]> c_pad_0 = const()[name = "
          @"string(\"c_pad_0\"), val = tensor<int32, [4]>([0, 0, 0, 0])];\n"
          @"            tensor<int32, [2]> c_dilations_0 = const()[name = "
          @"string(\"c_dilations_0\"), val = tensor<int32, [2]>([1, 1])];\n"
          @"            int32 c_groups_0 = const()[name = "
          @"string(\"c_groups_0\"), val = int32(1)];\n"
          @"            string x_to_fp16_dtype_0 = const()[name = "
          @"string(\"x_to_fp16_dtype_0\"), val = string(\"fp16\")];\n"];
  [m appendFormat:
          @"            tensor<fp16, [1, %d, 1, %d]> x_to_fp16 = cast(dtype = "
          @"x_to_fp16_dtype_0, x = x)[name = string(\"cast_in\")];\n",
          ch, sp];
  NSUInteger cs = 64 + ch * ch * 2;
  NSString *prev = @"x_to_fp16";
  for (int i = 0; i < depth; i++) {
    [m appendFormat:
            @"            tensor<fp16, [%d, %d, 1, 1]> W%d = const()[name = "
            @"string(\"W%d\"), val = tensor<fp16, [%d, %d, 1, "
    ...
    [m appendFormat:
            @"            tensor<fp16, [1, %d, 1, %d]> %@ = conv(dilations = "
            @"c_dilations_0, groups = c_groups_0, pad = c_pad_0, pad_type = "
            @"c_pad_type_0, strides = c_strides_0, weight = W%d, x = %@)[name "
            @"= string(\"%@\")];\n",
            ch, sp, out, i, prev, out];
    prev = out;
  }
  [m appendString:@"            string to_fp32 = const()[name = "
                  @"string(\"to_fp32\"), val = string(\"fp32\")];\n"];
  [m appendFormat:@"            tensor<fp32, [1, %d, 1, %d]> c = cast(dtype = "
                  @"to_fp32, x = %@)[name = string(\"cast_out\")];\n",
                  ch, sp, prev];
  [m appendString:@"        } -> (c);\n}\n"];
  return m;
}
...
double bench(int ch, int sp, int depth) {
  @autoreleasepool {
    ...
    [milData writeToFile:[td stringByAppendingPathComponent:@"model.mil"]
              atomically:YES];
    [wb writeToFile:[td stringByAppendingPathComponent:@"weights/weight.bin"]
         atomically:YES];
    ...
    id wI = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        AIO, @selector(objectWithIOSurface:), ioI);
...
int main() {
  ...
  C cf[] = {
      {512, 64, 32},  {512, 64, 48},  {512, 64, 64},  {512, 64, 96},
      {512, 64, 128}, {256, 64, 64},  {256, 64, 128}, {256, 64, 256},
      {384, 64, 64},  {384, 64, 128},
  };
  for (int i = 0; i < 10; i++) {
    int c = cf[i].c, s = cf[i].s, d = cf[i].d;
    double w = (double)c * c * 2 * d / 1024 / 1024,
           gf = 2.0 * c * c * s * d / 1e9;
    char l[64];
    snprintf(l, 64, "%dx conv %dch sp%d", d, c, s);
    double ms = bench(c, s, d);
    double tf = ms > 0 ? gf / ms : 0;
    if (ms > 0)
      printf("%-28s %6.1f  %6.2f  %7.3f ms %6.2f  %5.1f%%\n", l, w, gf, ms, tf,
             tf / 0.019 * 100);
    else
      printf("%-28s %6.1f  %6.2f  FAIL(%.0f)\n", l, w, gf, ms);
  }
  return 0;
}
