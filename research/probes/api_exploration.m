...
static __attribute__((unused)) double ticksToMs(uint64_t t) {
...
int main() {
  @autoreleasepool {
    ...
    if (@available(macOS 14.0, *)) {
      // Read the spec data
      NSString *specPath =
          [[compiled path] stringByAppendingPathComponent:@"coremldata.bin"];
      if (![fm fileExistsAtPath:specPath]) {
        specPath =
            [[compiled path] stringByAppendingPathComponent:@"model.mlmodel"];
      }
      ...
      if (assetClass) {
        ...
        unsigned int count;
        Method *methods =
            class_copyMethodList(object_getClass(assetClass), &count);
    ...
    // === Approach 2: Read a .mlmodelc, extract MIL, feed to
    ...
    if ([fm fileExistsAtPath:milPath]) {
      NSString *milText =
          [NSString stringWithContentsOfFile:milPath
                                    encoding:NSUTF8StringEncoding
                                       error:nil];
      printf("\n=== model.mil contents (first 2000 chars) ===\n");
      printf("%s\n", [[milText substringToIndex:MIN(2000, [milText length])]
                         UTF8String]);
    }
    ...
    while ((f = [en nextObject])) {
      if ([f hasSuffix:@".espresso.net"] || [f hasSuffix:@".hwx"] ||
          [f hasSuffix:@".mil"]) {
        printf("  FOUND: %s\n", [f UTF8String]);
      }
    }

    // === Approach 3: Try _ANEInMemoryModelDescriptor with actual MIL from
    // compiled model ===
    ...
    if (ANEInMemDesc) {
      ...
      unsigned int count;
      Method *methods =
          class_copyMethodList(object_getClass(ANEInMemDesc), &count);
      for (unsigned int i = 0; i < count; i++) {
