...
static const NSUInteger kANEQoS = 21;
static const int kWarmupIters = 5;
static const int kBenchIters = 30;
static id g_client;
...
static id makeRequest(IOSurfaceRef ioIn, IOSurfaceRef ioOut) {
  id wIn = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
      AIO, @selector(objectWithIOSurface:), ioIn);
  id wOut = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
      AIO, @selector(objectWithIOSurface:), ioOut);
...
double bench(const char *path, int ch, int sp) {
  @autoreleasepool {
    ...
    NSURL *compiled = [MLModel
        compileModelAtURL:[NSURL fileURLWithPath:[NSString
                                                     stringWithUTF8String:path]]
                    error:&e];
    if (e)
      return -1;
    id model = ((id (*)(Class, SEL, id, id))objc_msgSend)(
        AM, @selector(modelAtURL:key:), compiled, @"s");
    ...
    ok = ((BOOL (*)(id, SEL, id, id, NSUInteger, NSError **))objc_msgSend)(
        g_client, @selector(loadModel:options:qos:error:), model, @{}, kANEQoS,
        &e);
    if (!ok)
      return -3;
    ...
    ((void (*)(id, SEL, id, id, NSUInteger, NSError **))objc_msgSend)(
        g_client, @selector(unloadModel:options:qos:error:), model, @{},
        kANEQoS, &e);
    CFRelease(ioIn);
    CFRelease(ioOut);
    return ms;
...
int main() {
  ...
  S sizes[] = {{256, 64},  {512, 64},  {1024, 64}, {2048, 64}, {3072, 64},
               {4096, 64}, {5120, 64}, {6144, 64}, {8192, 32}};
