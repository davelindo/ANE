// test_weight_reload.m — Can we skip recompilation by rewriting weight blobs on
// disk? Compile a conv kernel with weights A, eval, verify output. Overwrite
// weights/weight.bin in tmpDir with weights B. unloadWithQoS: then loadWithQoS:
// (no recompile). Eval again — if output matches B @ x, compilation bottleneck
// is eliminated.
  ...
  return [NSString
      stringWithFormat:
          @"program(1.3)\n"
           ...
           "        string to16 = const()[name=string(\"to16\"), "
           "val=string(\"fp16\")];\n"
           "        tensor<fp16, [1,%d,1,%d]> x16 = "
           "cast(dtype=to16,x=x)[name=string(\"cin\")];\n"
           "        tensor<fp16, [%d,%d,1,1]> W = const()[name=string(\"W\"), "
           "val=tensor<fp16, "
           "[%d,%d,1,1]>(BLOBFILE(path=string(\"@model_path/weights/"
           "weight.bin\"), offset=uint64(64)))];\n"
           "        tensor<fp16, [1,%d,1,%d]> y16 = "
           "conv(dilations=dl,groups=gr,pad=pd,pad_type=pt,strides=st,weight=W,"
           "x=x16)"
           "[name=string(\"conv\")];\n"
           "        string to32 = const()[name=string(\"to32\"), "
           "val=string(\"fp32\")];\n"
           "        tensor<fp32, [1,%d,1,%d]> y = "
           "cast(dtype=to32,x=y16)[name=string(\"cout\")];\n"
           "    } -> (y);\n"
           "}\n",
...
int main() {
  @autoreleasepool {
    ...
    id desc = ((id (*)(Class, SEL, id, id, id))objc_msgSend)(
        g_D, @selector(modelWithMILText:weights:optionsPlist:), milData,
        weights, nil);
    if (!desc) {
      printf("FAIL: desc=NULL\n");
      return 1;
    }
    ...
    [milData writeToFile:[td stringByAppendingPathComponent:@"model.mil"]
              atomically:YES];
    [wdataA
        writeToFile:[td stringByAppendingPathComponent:@"weights/weight.bin"]
         atomically:YES];
    ...
    printf("  Output A[%d..%d]: [%.4f, %.4f, %.4f, %.4f]\n", CH * SP - 4,
           CH * SP - 1, outA[CH * SP - 4], outA[CH * SP - 3], outA[CH * SP - 2],
           outA[CH * SP - 1]);
    // Save copy
    ...
    printf("\n=== Step 3: Overwrite weight.bin with B (3x identity), "
           "unload+load ===\n");
    ...
    ok = ((BOOL (*)(id, SEL, unsigned int, NSError **))objc_msgSend)(
    ...
    wI = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioIn);
    wO = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioOut);
    ...
    ok = ((BOOL (*)(id, SEL, unsigned int, id, id, NSError **))objc_msgSend)(
        ...
        &e);
    if (!ok) {
      printf("FAIL: eval after reload: %s\n",
             e ? [[e description] UTF8String] : "?");
      return 1;
    }
    ...
    printf("  Output B[%d..%d]: [%.4f, %.4f, %.4f, %.4f]\n", CH * SP - 4,
           CH * SP - 1, outB[CH * SP - 4], outB[CH * SP - 3], outB[CH * SP - 2],
           outB[CH * SP - 1]);
    ...
    if (changed && correct_3x) {
      ...
      printf(
          "  ANE cached the compiled model — weights baked at compile time.\n");
