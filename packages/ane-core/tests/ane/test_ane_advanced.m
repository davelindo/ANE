...
int main() {
  @autoreleasepool {
    ...
    for (int i = 0; event_classes[i]; i++) {
      Class cls =
          NSClassFromString([NSString stringWithUTF8String:event_classes[i]]);
      if (!cls)
        continue;
      @try {
    ...
    // === Part 3: Compile working kernel for weightsBuffer + procedureIndex
    // tests ===
    ...
    *(uint32_t *)(blob + 80) = 128;
    ...
    NSData *wdata = [NSData dataWithBytesNoCopy:blob
                                         length:tot
                                   freeWhenDone:YES];
    ...
    id desc = ((id (*)(Class, SEL, id, id, id))objc_msgSend)(
        g_D, @selector(modelWithMILText:weights:optionsPlist:), md, @{
    ...
    id req0 = ((id (*)(Class, SEL, id, id, id, id, id, id, id))objc_msgSend)(
        g_AR,
    ...
    id wW = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioW);

    wI = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioIn);
    wO = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioOut);
    id req_wb = ((id (*)(Class, SEL, id, id, id, id, id, id, id))objc_msgSend)(
        g_AR,
        @selector(requestWithInputs:
                       inputIndices:outputs:outputIndices:weightsBuffer
                                   :perfStats:procedureIndex:),
    ...
    if (req_wb) {
      ...
      if (ok) {
        ...
        if (changed)
          printf(" (%s)",
                 is_3x ? "matches 3x — WORKS!" : "but not 3x as expected");
        printf("\n");
        IOSurfaceUnlock(ioOut, kIOSurfaceLockReadOnly, NULL);
      }
    }
    ...
    for (int pi = 0; pi < 16; pi++) {
      ...
      wO = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
          g_AIO, @selector(objectWithIOSurface:), ioOut);
      id req_p = ((id (*)(Class, SEL, id, id, id, id, id, id, id))objc_msgSend)(
          g_AR,
          @selector(requestWithInputs:
                         inputIndices:outputs:outputIndices:weightsBuffer
                                     :perfStats:procedureIndex:),
      ...
      printf("  procIdx %2d: %s%s\n", pi, ok ? "OK" : "FAIL",
             !ok && e ? [NSString
                            stringWithFormat:@" (%@)", [e localizedDescription]]
                            .UTF8String
                      : "");
    }
