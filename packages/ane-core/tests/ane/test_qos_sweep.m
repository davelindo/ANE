  ...
  @autoreleasepool {
    ...
    *(uint32_t *)(blob + 80) = 128;
    ...
    for (int i = 0; i < CH * CH; i++)
      wp[i] = (_Float16)(0.01f * (i % 100 - 50));
    NSData *blob_owner = [NSData dataWithBytesNoCopy:blob
                                              length:tot
                                        freeWhenDone:YES];
    (void)blob_owner;
    ...
    printf("Kernel: %dx%d conv, spatial=%d (%.1f MFLOPS)\n", CH, CH, SP,
           2.0 * CH * CH * SP / 1e6);
    printf("%4s %10s %10s %10s %10s  %s\n", "QoS", "Compile", "Load", "Eval(1)",
           "Eval(avg10)", "Status");

    unsigned int qos_values[] = {0,  1,  5,  10, 15, 17, 19, 21, 25,
                                 31, 33, 40, 47, 50, 55, 60, 63};
    ...
    for (int qi = 0; qi < n_qos; qi++) {
      ...
      NSDictionary *weights_q = @{
        @"@model_path/weights/weight.bin" : @{@"offset" : @0, @"data" : wdata_q}
      };
      ...
      BOOL eok =
          ((BOOL (*)(id, SEL, unsigned int, id, id, NSError **))objc_msgSend)(
              mdl, @selector(evaluateWithQoS:options:request:error:), qos, @{},
              req, &e);
