...
CKPT_PATH = os.environ.get(
    "ANE_CKPT_PATH",
    str(REPO_ROOT / ".local" / "checkpoints" / "ane_stories110M_ckpt.bin"),
)
TOKENIZER_PATH = os.environ.get(
    "ANE_TOKENIZER_PATH",
    str(REPO_ROOT / ".local" / "models" / "tokenizer.bin"),
)
LEGACY_TOKENIZER_PATH = str(REPO_ROOT / "assets" / "models" / "tokenizer.bin")
    ...
    if _tokenizer is None:
        try:
            ...
            if (
                ...
                and Path(LEGACY_TOKENIZER_PATH).exists()
            ):
                try:
                    _tokenizer = Tokenizer(LEGACY_TOKENIZER_PATH)
                    S.logs.append(f"[gen] tokenizer fallback: {LEGACY_TOKENIZER_PATH}")
                    return _tokenizer
                except Exception as fallback_error:
                    S.logs.append(f"[gen] tokenizer fallback failed: {fallback_error}")
    ...
    try:
        with open(path, "rb") as f:
            ...
            for L in range(NLAYERS):
                W[f"Wq{L}"] = (
                    np.frombuffer(f.read(wq_sz * 4), dtype=np.float32)
                    .reshape(DIM, DIM)
                    .copy()
                )
                W[f"Wk{L}"] = (
                ...
                W[f"Wo{L}"] = (
                    np.frombuffer(f.read(wo_sz * 4), dtype=np.float32)
                    .reshape(DIM, DIM)
                    .copy()
                )
                W[f"W1_{L}"] = (
                    np.frombuffer(f.read(w1_sz * 4), dtype=np.float32)
                    .reshape(HIDDEN, DIM)
                    .copy()
                )
                W[f"W2_{L}"] = (
                    np.frombuffer(f.read(w2_sz * 4), dtype=np.float32)
                    .reshape(DIM, HIDDEN)
                    .copy()
                )
                W[f"W3_{L}"] = (
                    np.frombuffer(f.read(w3_sz * 4), dtype=np.float32)
                    .reshape(HIDDEN, DIM)
                    .copy()
                )
...
RE_CONFIG = re.compile(
    r"dim=(\d+) hidden=(\d+) heads=(\d+) seq=(\d+) vocab=(\d+) layers=(\d+)"
)
RE_PARAMS = re.compile(
    r"Params: ([\d.]+)M \(transformer ([\d.]+)M \+ embed ([\d.]+)M\)"
)
...
RE_BATCH = re.compile(
    r"\[batch (\d+): compile=([\d.]+)ms train=([\d.]+)ms \(([\d.]+)ms/step\) compiles=(\d+)\]"
)
RE_TIMING = re.compile(
    r"ane=([\d.]+) io=([\d.]+) cls=([\d.]+) elem=([\d.]+) rms=([\d.]+) cblas_wait=([\d.]+)"
)
RE_RESTART = re.compile(r"\[exec\(\) restart step (\d+)")
RE_RESUME = re.compile(r"\[RESUMED step (\d+), loss=([\d.]+)\]")
RE_FLOPS = re.compile(
    r"FLOPs/step: fwd=([\d.]+)M bwd_dx=([\d.]+)M bwd_dW=([\d.]+)M sdpa_bwd=([\d.]+)M total=([\d.]+)M"
)
...
RE_EFFICIENCY = re.compile(
    r"(Total steps|Wall time|Compile time|Train time|Avg compile|Avg train|ANE TFLOPS|Total TFLOPS|ANE utilization):?\s+(.+)"
)
...
def parse_line(line):
    ...
    if m:
        S.model_config = dict(
            zip(
                ["dim", "hidden", "heads", "seq", "vocab", "layers"],
                map(int, m.groups()),
            )
        )
        return
    ...
    if m:
        ...
        S.compiles = int(m[5])
        S.compile_pct = (
            100 * compile_ms / (compile_ms + train_ms)
            if compile_ms + train_ms > 0
            else 0
        )
        return
    m = RE_TIMING.search(line)
    if m:
        S.component_timing = dict(
            zip(
                ["ane", "io", "cls", "elem", "rms", "cblas_wait"],
                map(float, m.groups()),
            )
        )
        return
...
def draw(term):
    ...
    sr = row
    step_str = f"{S.step}" + (
        f"/{S.total_steps}" if S.total_steps and S.total_steps < 999999 else ""
    )
    put(
        sr,
        mid_x + 1,
        f" Step: {step_str}  Loss: {S.loss:.4f}" if S.loss else " Step: --",
        term.yellow,
    )
    sr += 1
    put(
        ...
        mid_x + 1,
        (
            f" Best: {S.best_loss:.4f}   ms/step: {S.ms_per_step:.1f}"
    ...
    if ane_tflops:
        put(
            sr,
            mid_x + 1,
            f" ANE: {ane_tflops:.2f}T  Compile: {S.compile_pct:.0f}%  Util: {ane_util:.1f}%",
        )
    else:
    ...
    if ct:
        put(
            sr,
            mid_x + 1,
            f' ane={ct.get("ane", 0):.1f} io={ct.get("io", 0):.1f} cls={ct.get("cls", 0):.1f} elem={ct.get("elem", 0):.1f}',
        )
        sr += 1
        put(
            sr,
            mid_x + 1,
            f' rms={ct.get("rms", 0):.1f} cblas_wait={ct.get("cblas_wait", 0):.1f} ms/step',
        )
        sr += 1
    pw = S.power
    if any(pw.values()):
        ...
        put(
            sr,
            mid_x + 1,
            f' ANE: {pw["ane"]:.1f}W  CPU: {pw["cpu"]:.1f}W  GPU: {pw["gpu"]:.1f}W',
            term.magenta,
        )
        sr += 1
    if S.batch_num:
    ...
    if has_sysmetrics:
        ...
        mem_vals = (
            list(S.proc_mem_mb_history)
            if S.proc_mem_mb_history
            else list(S.mem_mb_history)
        )
        cpu_lines = braille_chart(
            cpu_vals, left_w - 1, power_h, label_fmt="{:.0f}", y_range=(0, 100)
        )
    ...
    if gen_text or gen_status == "generating":
        status_tag = (
            " (generating...)" if gen_status == "generating" else f" (step {gen_step})"
        )
        put(
            row,
            0,
...
def main():
    ...
    parser.add_argument(
        "--no-generate", action="store_true", help="Disable text generation"
    )
    parser.add_argument(
        "--steps", type=int, default=10000, help="Total steps (default: 10000)"
    )
                    ...
                    if not data:
                        if train_proc.poll() is not None:
                            try:
                                ...
                                pass
                            S.logs.append(
                                "[dashboard] Training finished. Press q to exit."
                            )
                            train_finished = True
                        continue
