...
static NSData *build_blob(const float *w, int oc, int ic) {
  ...
  *(uint32_t *)(buf + 80) = 128;
  ...
  for (int i = 0; i < oc * ic; i++)
    fp16[i] = (_Float16)w[i]; // layout A: row-major [oc, ic]
  ...
  return [NSString
      stringWithFormat:
          ...
          @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
           ...
           "string(\"@model_path/weights/wq.bin\"), offset = uint64(64)))];\n"
           "        tensor<fp16, [%d, %d, 1, 1]> Wk = const()[name = "
           "string(\"Wk\"), "
           "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
           "string(\"@model_path/weights/wk.bin\"), offset = uint64(64)))];\n"
           "        tensor<fp16, [%d, %d, 1, 1]> Wv = const()[name = "
           "string(\"Wv\"), "
           "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
           "string(\"@model_path/weights/wv.bin\"), offset = uint64(64)))];\n"
           "        tensor<fp16, [1, %d, 1, %d]> q = conv(dilations = dl, "
           "groups = gr, pad = pd, "
           "pad_type = pt, strides = st, weight = Wq, x = x16)[name = "
           "string(\"cq\")];\n"
           "        tensor<fp16, [1, %d, 1, %d]> k = conv(dilations = dl, "
           "groups = gr, pad = pd, "
           "pad_type = pt, strides = st, weight = Wk, x = x16)[name = "
           "string(\"ck\")];\n"
           "        tensor<fp16, [1, %d, 1, %d]> v = conv(dilations = dl, "
           "groups = gr, pad = pd, "
           "pad_type = pt, strides = st, weight = Wv, x = x16)[name = "
           "string(\"cv\")];\n"
           "        int32 ax = const()[name = string(\"ax\"), val = "
           "int32(1)];\n"
           "        bool inter = const()[name = string(\"il\"), val = "
           "bool(false)];\n"
           "        tensor<fp16, [1, %d, 1, %d]> qkv = concat(axis = ax, "
           "interleave = inter, values = (q, k, v))[name = string(\"cat\")];\n"
           "        string d2 = const()[name = string(\"d2\"), val = "
           "string(\"fp32\")];\n"
           "        tensor<fp32, [1, %d, 1, %d]> y = cast(dtype = d2, x = "
           "qkv)[name = string(\"co\")];\n"
           "    } -> (y);\n}\n",
          DIM, SEQ, DIM, SEQ, DIM, DIM, DIM, DIM, // Wq
          DIM, DIM, DIM, DIM,                     // Wk
          DIM, DIM, DIM, DIM,                     // Wv
          DIM, SEQ,                               // q
          DIM, SEQ,                               // k
  ...
  return [NSString
      stringWithFormat:
          ...
          @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
           ...
           "string(\"W\"), "
           "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
           "string(\"@model_path/weights/w.bin\"), offset = uint64(64)))];\n"
           "        string pt = const()[name = string(\"pt\"), val = "
           "string(\"valid\")];\n"
           "        tensor<int32, [2]> st = const()[name = string(\"st\"), val "
           "= tensor<int32, [2]>([1, 1])];\n"
...
int main() {
  @autoreleasepool {
    ...
    printf("Separate Q,K,V: %s %s %s\n", kQ.model ? "OK" : "FAIL",
           kK.model ? "OK" : "FAIL", kV.model ? "OK" : "FAIL");

    if (!kFused.model || !kQ.model)
      goto done;

    // IOSurfaces
    size_t in_bytes = DIM * SEQ * 4, out1_bytes = DIM * SEQ * 4,
           out3_bytes = DIM * 3 * SEQ * 4;
    IOSurfaceRef ioIn = make_surface(in_bytes);
    IOSurfaceRef ioFused = make_surface(out3_bytes);
    IOSurfaceRef ioQ = make_surface(out1_bytes), ioK = make_surface(out1_bytes),
                 ioV = make_surface(out1_bytes);
    ...
    for (int c = 0; c < DIM; c++)
      for (int t = 0; t < SEQ; t++) {
        float d1 = fabsf(fo[c * SEQ + t] - qo[c * SEQ + t]);
        if (d1 > dq)
          dq = d1;
        float d2 = fabsf(fo[(DIM + c) * SEQ + t] - ko[c * SEQ + t]);
        if (d2 > dk)
          dk = d2;
        float d3 = fabsf(fo[(DIM * 2 + c) * SEQ + t] - vo[c * SEQ + t]);
        if (d3 > dv)
          dv = d3;
      }
    ...
    printf("\nFused vs Separate: dQ=%.6f dK=%.6f dV=%.6f → %s\n", dq, dk, dv,
           (dq < 0.001f && dk < 0.001f && dv < 0.001f) ? "PASS" : "FAIL");
