// Decomposed causal attention: Q@K^T on ANE, mask+softmax on CPU, scores@V on
// ANE This gives us causal masking with ANE acceleration for the matmuls
...
static Kern compile_mil(NSString *mil) {
  ...
  id desc = ((id (*)(Class, SEL, id, id, id))objc_msgSend)(
      g_D, @selector(modelWithMILText:weights:optionsPlist:), md, @{}, nil);
  if (!desc) {
    printf("desc=NULL\n");
    return k;
  }
  ...
  [[NSFileManager defaultManager] createDirectoryAtPath:td
...
int main() {
  @autoreleasepool {
    ...
    IOSurfaceRef ioQ = make_surface(qkv_bytes), ioK = make_surface(qkv_bytes),
                 ioV = make_surface(qkv_bytes);
    IOSurfaceLock(ioQ, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioQ), Q, qkv_bytes);
    IOSurfaceUnlock(ioQ, 0, NULL);
    IOSurfaceLock(ioK, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioK), K, qkv_bytes);
    IOSurfaceUnlock(ioK, 0, NULL);
    IOSurfaceLock(ioV, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioV), V, qkv_bytes);
    ...
    // === Run non-causal SDPA ===
    {
      ...
      if (!ane_eval(&kSDPA, ins, 3, ioOut_sdpa)) {
        printf("SDPA eval FAIL\n");
        goto done;
      }
    }
    ...
    // Step 1: Q@K^T on ANE
    {
      ...
      if (!ane_eval(&kQKT, ins, 2, ioScores)) {
        printf("Q@K^T eval FAIL\n");
        goto done;
      }
    }
    ...
    // Step 3: softmax_scores @ V on ANE
    {
      ...
      if (!ane_eval(&kSV, ins, 2, ioOut_decomp)) {
        printf("scores@V eval FAIL\n");
        goto done;
      }
    }
      ...
      for (int h = 0; h < HEADS; h++)
        for (int t = 0; t < SEQ; t++) {
          ...
          for (int d = 0; d < HD; d++) {
            float ref = 0;
            for (int t2 = 0; t2 <= t; t2++)
              ref += scores[t2] * (float)V[h * SEQ * HD + t2 * HD + d];
    ...
    int N = 500;
    {
      ...
      // Warmup
      for (int i = 0; i < 10; i++)
        ane_eval(&kSDPA, ins, 3, ioOut_sdpa);
    ...
    CFRelease(ioV);
    CFRelease(ioScores);
    CFRelease(ioOut_sdpa);
    CFRelease(ioOut_decomp);
    free(Q);
    free(K);
    free(V);
