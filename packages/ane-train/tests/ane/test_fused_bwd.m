  ...
  @autoreleasepool {
    ...
    NSString *mil =
        [NSString stringWithFormat:
                      ...
                      @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
                       ...
                       "= "
                       "string(\"fp16\")];\n"
                       "        tensor<fp16, [1, %d, 1, %d]> x16 = cast(dtype "
                       "= d1, x = "
                       "x)[name = string(\"cx\")];\n"
                       // Slice: dh1 = x16[:, 0:HIDDEN, :, :], dh3 = x16[:,
                       // HIDDEN:2*HIDDEN, :, :]
                       "        tensor<int32, [4]> b1 = const()[name = "
                       "string(\"b1\"), "
                       "val = tensor<int32, [4]>([0, 0, 0, 0])];\n"
                       "        tensor<int32, [4]> s1 = const()[name = "
                       "string(\"s1\"), "
                       "val = tensor<int32, [4]>([1, %d, 1, %d])];\n"
                       "        tensor<fp16, [1, %d, 1, %d]> dh1 = "
                       "slice_by_size(x = "
                       "x16, begin = b1, size = s1)[name = string(\"sl1\")];\n"
                       "        tensor<int32, [4]> b3 = const()[name = "
                       "string(\"b3\"), "
                       "val = tensor<int32, [4]>([0, %d, 0, 0])];\n"
                       "        tensor<int32, [4]> s3 = const()[name = "
                       "string(\"s3\"), "
                       "val = tensor<int32, [4]>([1, %d, 1, %d])];\n"
                       "        tensor<fp16, [1, %d, 1, %d]> dh3 = "
                       "slice_by_size(x = "
                       "x16, begin = b3, size = s3)[name = string(\"sl3\")];\n"
                       // Conv: W1^T @ dh1, W3^T @ dh3
                       "        string pt = const()[name = string(\"pt\"), val "
                       "= "
                       "string(\"valid\")];\n"
                       "        tensor<int32, [2]> st = const()[name = "
                       "string(\"st\"), "
                       "val = tensor<int32, [2]>([1, 1])];\n"
                       "        tensor<int32, [4]> pd = const()[name = "
                       "string(\"pd\"), "
                       "val = tensor<int32, [4]>([0, 0, 0, 0])];\n"
                       "        tensor<int32, [2]> dl = const()[name = "
                       "string(\"dl\"), "
                       "val = tensor<int32, [2]>([1, 1])];\n"
                       "        int32 gr = const()[name = string(\"gr\"), val "
                       "= "
                       "int32(1)];\n"
                       // W1^T: [DIM, HIDDEN, 1, 1]  (transposed from [HIDDEN,
                       // DIM])
                       "        tensor<fp16, [%d, %d, 1, 1]> W1t = "
                       "const()[name = "
                       "string(\"W1t\"), "
                       "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
                       "string(\"@model_path/weights/w1t.bin\"), offset = "
                       "uint64(64)))];\n"
                       "        tensor<fp16, [%d, %d, 1, 1]> W3t = "
                       "const()[name = "
                       "string(\"W3t\"), "
                       "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
                       "string(\"@model_path/weights/w3t.bin\"), offset = "
                       "uint64(64)))];\n"
                       "        tensor<fp16, [1, %d, 1, %d]> dx1 = "
                       "conv(dilations = dl, "
                       "groups = gr, pad = pd, "
                       "pad_type = pt, strides = st, weight = W1t, x = "
                       "dh1)[name = "
                       "string(\"cv1\")];\n"
                       "        tensor<fp16, [1, %d, 1, %d]> dx3 = "
                       "conv(dilations = dl, "
                       "groups = gr, pad = pd, "
                       "pad_type = pt, strides = st, weight = W3t, x = "
                       "dh3)[name = "
                       "string(\"cv3\")];\n"
                       // Add
                       "        tensor<fp16, [1, %d, 1, %d]> sum = add(x = "
                       "dx1, y = "
                       "dx3)[name = string(\"ad\")];\n"
                       "        string d2 = const()[name = string(\"d2\"), val "
                       "= "
                       "string(\"fp32\")];\n"
                       "        tensor<fp32, [1, %d, 1, %d]> y = cast(dtype = "
                       "d2, x = "
                       "sum)[name = string(\"co\")];\n"
                       "    } -> (y);\n}\n",
                      HIDDEN * 2, SEQ, HIDDEN * 2, SEQ, HIDDEN, SEQ, HIDDEN,
                      SEQ,                              // slice1
    ...
    NSDictionary *wd = @{
      @"@model_path/weights/w1t.bin" :
          @{@"offset" : @0, @"data" : build_blob_t(W1, HIDDEN, DIM)},
      @"@model_path/weights/w3t.bin" :
          @{@"offset" : @0, @"data" : build_blob_t(W3, HIDDEN, DIM)}
    };
    ...
    for (int i = 0; i < SEQ * HIDDEN; i++) {
      dh1[i] = 0.01f * sinf(i * 0.007f);
      dh3[i] = 0.01f * cosf(i * 0.011f);
    }

    IOSurfaceRef ioI = make_surface(HIDDEN * 2 * SEQ * 4),
                 ioO = make_surface(DIM * SEQ * 4);
    IOSurfaceLock(ioI, 0, NULL);
    float *dst = (float *)IOSurfaceGetBaseAddress(ioI);
    // Channel-first: channels 0..HIDDEN-1 = dh1, channels HIDDEN..2*HIDDEN-1 =
    // dh3
    for (int t = 0; t < SEQ; t++) {
      for (int c = 0; c < HIDDEN; c++)
        dst[c * SEQ + t] = dh1[t * HIDDEN + c];
      for (int c = 0; c < HIDDEN; c++)
        dst[(HIDDEN + c) * SEQ + t] = dh3[t * HIDDEN + c];
    }
