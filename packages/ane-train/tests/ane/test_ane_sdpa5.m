...
static NSString *build_inline_causal_mask(int s) {
  ...
  return [NSString
      stringWithFormat:@"tensor<fp16, [1, 1, %d, %d]>([%@])", s, s, vals];
}
...
static void cleanup_model(Model *m) {
  ...
  ((BOOL (*)(id, SEL, unsigned int, NSError **))objc_msgSend)(
      m->model, @selector(unloadWithQoS:error:), 21, &e);
...
int main() {
  @autoreleasepool {
    ...
    memcpy(IOSurfaceGetBaseAddress(ioQ), Q, bytes);
    IOSurfaceUnlock(ioQ, 0, NULL);
    IOSurfaceLock(ioK, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioK), K, bytes);
    IOSurfaceUnlock(ioK, 0, NULL);
    IOSurfaceLock(ioV, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioV), V, bytes);
    ...
    for (int h = 0; h < HEADS; h++)
      for (int t = 0; t < SEQ; t++) {
        ...
        for (int t2 = 0; t2 <= t; t2++) {
          float s = 0;
          for (int d = 0; d < HD; d++)
            s += (float)Q[h * SEQ * HD + t * HD + d] *
                 (float)K[h * SEQ * HD + t2 * HD + d];
          s *= scale;
          scores[t2] = s;
          if (s > maxs)
            maxs = s;
        ...
        for (int d = 0; d < HD; d++) {
          float r = 0;
          for (int t2 = 0; t2 <= t; t2++)
            r += scores[t2] * (float)V[h * SEQ * HD + t2 * HD + d];
    ...
    void (^eval_and_compare)(const char *, Model *, int nInputs,
                             IOSurfaceRef *) = ^(const char *label, Model *m,
                                                 int nInputs,
                                                 IOSurfaceRef *inputs) {
      ...
      for (int i = 0; i < nInputs; i++) {
        [inArr
            addObject:((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
      ...
      if (!ok) {
        printf("  %s: eval FAIL: %s\n", label,
               e ? [[[e localizedDescription]
                       substringToIndex:MIN(200, (int)[[e localizedDescription]
                                                     length])] UTF8String]
                 : "");
        CFRelease(ioO);
        return;
      ...
      for (int i = 0; i < total; i++) {
        float v = (float)out[i];
        float d1 = fabsf(v - cpu_causal[i]);
        if (d1 > dc)
          dc = d1;
        float d2 = fabsf(v - cpu_nocausal[i]);
        if (d2 > dnc)
          dnc = d2;
      }
      ...
      NSString *mil = [NSString
          stringWithFormat:
              ...
              @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
               ...
               "tensor<fp16, [1, %d, %d, %d]> k, tensor<fp16, [1, %d, %d, %d]> "
               "v) {\n"
      ...
      NSString *mil = [NSString
          stringWithFormat:
              ...
              @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
               ...
               "        tensor<fp16, [1, %d, %d, %d]> att = "
               "scaled_dot_product_attention("
               "query = q, key = k, value = v, attn_mask = mask)[name = "
               "string(\"sdpa\")];\n"
               "    } -> (att);\n}\n",
              HEADS, SEQ, HD, HEADS, SEQ, HD, HEADS, SEQ, HD,
              [NSString
                  stringWithFormat:@"tensor<fp16, [1, 1, %d, %d]>", SEQ, SEQ],
              maskStr, HEADS, SEQ, HD];
      Model m = compile_model(mil, nil);
      if (m.model) {
        IOSurfaceRef ins[] = {ioQ, ioK, ioV};
      ...
      NSString *mil = [NSString
          stringWithFormat:
              ...
              @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
               ...
               "tensor<fp16, [1, %d, %d, %d]> k, tensor<fp16, [1, %d, %d, %d]> "
               "v) {\n"
               "        tensor<fp16, [1, 1, %d, %d]> mask = const()[name = "
               "string(\"mask\"), "
               ...
               "string(\"sdpa\")];\n"
               "    } -> (att);\n}\n",
              HEADS, SEQ, HD, HEADS, SEQ, HD, HEADS, SEQ, HD, SEQ, SEQ, SEQ,
              SEQ, HEADS, SEQ, HD];
      NSDictionary *wd = @{
        @"@model_path/weights/mask.bin" :
            @{@"offset" : @0, @"data" : build_mask_blob(SEQ)}
      };
