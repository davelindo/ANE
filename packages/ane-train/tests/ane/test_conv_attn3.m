  ...
  return [NSString
      stringWithFormat:
          ...
          @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
           ...
           "groups = gr, pad = pd, "
           "pad_type = pt, strides = st, weight = W, x = x)[name = "
           "string(\"cv\")];\n"
           "    } -> (y);\n}\n",
  ...
  @autoreleasepool {
    ...
    NSDictionary *qkt_wd = @{
      @"@model_path/weights/w.bin" :
          @{@"offset" : @0, @"data" : build_blob_raw(kw, kw_count)}
    };
    free(kw);
    ...
    NSDictionary *sv_wd = @{
      @"@model_path/weights/w.bin" :
          @{@"offset" : @0, @"data" : build_blob_raw(vw, vw_count)}
    };
    free(vw);
    ...
    if (!ane_eval(&kQKT, ins1, 1, ioScores)) {
    ...
    for (int h = 0; h < HEADS; h++)
      for (int t = 0; t < SEQ; t++) {
        ...
        for (int t2 = 0; t2 <= t; t2++) {
          float s = 0;
          for (int d = 0; d < HD; d++)
            s += Q[t * DIM + h * HD + d] * K[t2 * DIM + h * HD + d];
          s *= scale;
          sc2[t2] = s;
          if (s > maxs)
            maxs = s;
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int h = 0; h < HEADS; h++)
        for (int t = 0; t < SEQ; t++) {
          ...
          for (int t2 = 0; t2 < SEQ; t2++)
            s[(h * SEQ + t2) * SEQ + t] = (_Float16)(row[t2] / sum);
        }
      IOSurfaceUnlock(ioScores, 0, NULL);
      ane_eval(&kSV, ins2, 1, ioOut);
    }
