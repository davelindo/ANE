// backward.h — Backward pass using CPU matmul (correct gradients) + ANE
// optional
#pragma once
#include "forward.h"
#include "model.h"
...
static void cpu_accum_dW(float *dW, const float *dy, const float *x, int S,
...
static void cpu_rmsnorm_backward(float *dx, const float *dy, const float *x,
                                 const float *w, int S, int D) {
  for (int t = 0; t < S; t++) {
    float ss = 0;
    for (int i = 0; i < D; i++)
      ss += x[t * D + i] * x[t * D + i];
...
static void cpu_attention_backward(float *dq, float *dk, float *dv,
                                   const float *d_out, const float *q,
                                   const float *k, const float *v, int S,
  ...
  for (int h = 0; h < n_heads; h++) {
    for (int t = 0; t < S; t++) {
      ...
      for (int s = 0; s <= t; s++) {
        ...
        for (int i = 0; i < head_dim; i++)
          dv[s * D + h * head_dim + i] +=
              scores[s] * d_out[t * D + h * head_dim + i];
      }
    }
  }
  free(scores);
...
static void cpu_rope_backward(float *dq, float *dk, int S, int n_heads,
                              int head_dim) {
  for (int t = 0; t < S; t++)
    for (int h = 0; h < n_heads; h++)
...
#define ACCUM_NORM(grad, size)                                                 \
  do {                                                                         \
    for (size_t _i = 0; _i < (size_t)(size); _i++)                             \
      total_norm_sq += (double)(grad)[_i] * (grad)[_i];                        \
  } while (0)
  for (int l = 0; l < N_LAYERS; l++) {
    ACCUM_NORM(m->grad_wq[l], d * d);
    ACCUM_NORM(m->grad_wk[l], d * d);
    ACCUM_NORM(m->grad_wv[l], d * d);
    ACCUM_NORM(m->grad_wo[l], d * d);
    ACCUM_NORM(m->grad_w1[l], hd * d);
    ACCUM_NORM(m->grad_w2[l], d * hd);
    ACCUM_NORM(m->grad_w3[l], hd * d);
  }
  ACCUM_NORM(m->grad_wcls, vs * d);
  ACCUM_NORM(m->grad_emb, vs * d);
#undef ACCUM_NORM
  float total_norm = sqrtf((float)total_norm_sq);
  if (total_norm > max_norm) {
    float scale = max_norm / total_norm;
#define SCALE_GRAD(grad, size)                                                 \
  do {                                                                         \
    for (size_t _i = 0; _i < (size_t)(size); _i++)                             \
      (grad)[_i] *= scale;                                                     \
  } while (0)
    for (int l = 0; l < N_LAYERS; l++) {
      SCALE_GRAD(m->grad_wq[l], d * d);
      SCALE_GRAD(m->grad_wk[l], d * d);
      SCALE_GRAD(m->grad_wv[l], d * d);
      SCALE_GRAD(m->grad_wo[l], d * d);
      SCALE_GRAD(m->grad_w1[l], hd * d);
      SCALE_GRAD(m->grad_w2[l], d * hd);
      SCALE_GRAD(m->grad_w3[l], hd * d);
    }
    SCALE_GRAD(m->grad_wcls, vs * d);
    SCALE_GRAD(m->grad_emb, vs * d);
#undef SCALE_GRAD
  }
}
...
static void model_backward(Model *m, const int *tokens) {
  ...
  for (int t = 0; t < S - 1; t++) {
    float mx = -1e9f;
    for (int i = 0; i < vs; i++)
      if (m->logits[t * vs + i] > mx)
        mx = m->logits[t * vs + i];
    float sm = 0;
    for (int i = 0; i < vs; i++)
      sm += expf(m->logits[t * vs + i] - mx);
    for (int i = 0; i < vs; i++)
  ...
  for (int l = N_LAYERS - 1; l >= 0; l--) {
    ...
    for (int t = 0; t < S; t++)
      for (int i = 0; i < hd; i++) {
        d_h1[t * hd + i] = d_silu[t * hd + i] * m->act_h3[l][t * hd + i] *
                           silu_backward(m->act_h1[l][t * hd + i]);
        d_h3[t * hd + i] =
            d_silu[t * hd + i] * silu_f(m->act_h1[l][t * hd + i]);
      }
    free(d_silu);
    ...
    // The input to FFN rmsnorm was the residual after attention = act_x[l] +
    // attn_residual We saved act_x[l] but the actual input to ffn_rmsnorm is x
    // after attention residual For a proper implementation we'd save this.
    // Approximate with act_x[l].
    cpu_rmsnorm_backward(dx_ffn_norm, dx_ffn_in, m->act_x[l], m->rms_ffn_w[l],
                         S, d);
    for (int i = 0; i < S * d; i++)
      dx[i] += dx_ffn_norm[i];
    free(dx_ffn_in);
    ...
    cpu_attention_backward(dq, dk, dv, d_attn_out, m->act_q[l], m->act_k[l],
                           m->act_v[l], S, nh, hdim);
    free(d_attn_out);
    ...
    cpu_rmsnorm_backward(dx_att_norm, dx_qkv, m->act_x[l], m->rms_att_w[l], S,
                         d);
    for (int i = 0; i < S * d; i++)
      dx[i] += dx_att_norm[i];
    free(dx_qkv);
...
static void model_adam_step(Model *m, float lr, float beta1, float beta2,
                            float eps) {
  m->adam_step++;
...
#define ADAM_UPDATE(param, grad, size)                                         \
  do {                                                                         \
    for (size_t _i = 0; _i < (size_t)(size); _i++) {                           \
      float g = (grad)[_i];                                                    \
