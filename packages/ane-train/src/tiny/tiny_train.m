...
// Pipeline: compile next kernels on background thread while ANE runs current
// batch Bypasses ANE 119-compile limit via exec() self-restart
  ...
  return [NSString
      stringWithFormat:
          ...
          @"string>({{\"coremlc-component-MIL\", \"3510.2.1\"}, "
           ...
           "        string d1 = const()[name = string(\"d1\"), val = "
           "string(\"fp16\")];\n"
           "        tensor<fp16, [1, %d, 1, %d]> x16 = cast(dtype = d1, x = "
           "x)[name = string(\"cx\")];\n"
           "        tensor<fp16, [%d, %d, 1, 1]> W = const()[name = "
           "string(\"W\"), "
           "val = tensor<fp16, [%d, %d, 1, 1]>(BLOBFILE(path = "
           "string(\"@model_path/weights/weight.bin\"), offset = "
           "uint64(64)))];\n"
           "        string pt = const()[name = string(\"pt\"), val = "
           "string(\"valid\")];\n"
           "        tensor<int32, [2]> st = const()[name = string(\"st\"), val "
           "= tensor<int32, [2]>([1, 1])];\n"
           "        tensor<int32, [4]> pd = const()[name = string(\"pd\"), val "
           "= tensor<int32, [4]>([0, 0, 0, 0])];\n"
           "        tensor<int32, [2]> dl = const()[name = string(\"dl\"), val "
           "= tensor<int32, [2]>([1, 1])];\n"
           "        int32 gr = const()[name = string(\"gr\"), val = "
           "int32(1)];\n"
           "        tensor<fp16, [1, %d, 1, %d]> y16 = conv(dilations = dl, "
           "groups = gr, pad = pd, "
           "pad_type = pt, strides = st, weight = W, x = x16)[name = "
           "string(\"cv\")];\n"
           "        string d2 = const()[name = string(\"d2\"), val = "
           "string(\"fp32\")];\n"
           "        tensor<fp32, [1, %d, 1, %d]> y = cast(dtype = d2, x = "
           "y16)[name = string(\"co\")];\n"
           "    } -> (y);\n}\n",
          in_ch, sp, in_ch, sp, out_ch, in_ch, out_ch, in_ch, out_ch, sp,
          out_ch, sp];
}
...
static Kern *compile_kern_with_blob(NSData *blob, int in_ch, int out_ch,
                                    int sp) {
  @autoreleasepool {
    ...
    id desc = ((id (*)(Class, SEL, id, id, id))objc_msgSend)(
        g_D, @selector(modelWithMILText:weights:optionsPlist:), milData, wd,
        nil);
    if (!desc)
      return NULL;
    ...
    id wI = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioI);
    id wO = ((id (*)(Class, SEL, IOSurfaceRef))objc_msgSend)(
        g_AIO, @selector(objectWithIOSurface:), ioO);
    id req = ((id (*)(Class, SEL, id, id, id, id, id, id, id))objc_msgSend)(
        g_AR,
        @selector(requestWithInputs:
                       inputIndices:outputs:outputIndices:weightsBuffer
...
static void ane_eval_k(Kern *k, const float *in, float *out, int in_ch,
                       int out_ch, int sp) {
...
static bool io_write_exact(FILE *f, const void *ptr, size_t item_size,
                           size_t count, const char *path, const char *what) {
  if (count == 0)
    return true;
  if (fwrite(ptr, item_size, count, f) == count)
    return true;
  fprintf(stderr, "Checkpoint write failed (%s, %s): %s\n", path, what,
          strerror(errno));
  return false;
}

static bool io_read_exact(FILE *f, void *ptr, size_t item_size, size_t count,
  ...
  if (got == count)
    return true;
  if (ferror(f)) {
    fprintf(stderr, "Checkpoint read failed (%s, %s): %s\n", path, what,
            strerror(errno));
  } else {
    fprintf(stderr, "Checkpoint truncated (%s, %s): read %zu of %zu items\n",
            path, what, got, count);
  }
  return false;
}

static bool io_close_checked(FILE *f, const char *path) {
  if (fclose(f) == 0)
    return true;
  fprintf(stderr, "Checkpoint close failed (%s): %s\n", path, strerror(errno));
  return false;
}

static bool save_checkpoint(const char *path, int step, float loss, int D,
                            int H, int S, int total_steps, float lr,
                            const float *W1, const float *W2, double cc,
                            double ct, double cw, int cs, int cb) {
  FILE *f = fopen(path, "wb");
  if (!f) {
    fprintf(stderr, "Checkpoint open for write failed (%s): %s\n", path,
            strerror(errno));
    return false;
  }
  ...
  size_t w1_count = (size_t)H * (size_t)D;
  size_t w2_count = (size_t)D * (size_t)H;

  if (!io_write_exact(f, &hdr, sizeof(hdr), 1, path, "header"))
    goto out;
  if (!io_write_exact(f, W1, sizeof(float), w1_count, path, "W1"))
    goto out;
  if (!io_write_exact(f, W2, sizeof(float), w2_count, path, "W2"))
    goto out;
  ok = true;

out:
  if (!io_close_checked(f, path))
    ok = false;
  return ok;
}

static bool load_checkpoint(const char *path, CkptHeader *hdr, float *W1,
                            float *W2, int H, int D) {
  FILE *f = fopen(path, "rb");
  if (!f)
    return false;

  bool ok = false;
  CkptHeader tmp_hdr = {0};
  size_t w1_count = (size_t)H * (size_t)D;
  size_t w2_count = (size_t)D * (size_t)H;
  float *tmp_w1 = (float *)malloc(w1_count * sizeof(float));
  float *tmp_w2 = (float *)malloc(w2_count * sizeof(float));
  if (!tmp_w1 || !tmp_w2) {
    fprintf(stderr, "Checkpoint alloc failed (%s)\n", path);
    goto out;
  }

  if (!io_read_exact(f, &tmp_hdr, sizeof(tmp_hdr), 1, path, "header"))
    goto out;
  if (tmp_hdr.D != D || tmp_hdr.H != H) {
    fprintf(
        stderr,
        "Checkpoint shape mismatch (%s): file D=%d H=%d, expected D=%d H=%d\n",
        path, tmp_hdr.D, tmp_hdr.H, D, H);
    goto out;
  }
  if (!io_read_exact(f, tmp_w1, sizeof(float), w1_count, path, "W1"))
    goto out;
  if (!io_read_exact(f, tmp_w2, sizeof(float), w2_count, path, "W2"))
    goto out;

  *hdr = tmp_hdr;
  memcpy(W1, tmp_w1, w1_count * sizeof(float));
  memcpy(W2, tmp_w2, w2_count * sizeof(float));
  ok = true;
...
int main(int argc, char *argv[]) {
  @autoreleasepool {
    ...
    g_compile_queue =
        dispatch_queue_create("ane.compile", DISPATCH_QUEUE_SERIAL);
    ...
    if (resuming) {
      cum_compile_ms = resume_hdr.cum_compile_ms;
      cum_train_ms = resume_hdr.cum_train_ms;
      cum_wall_ms = resume_hdr.cum_wall_ms;
      cum_steps = resume_hdr.cum_steps;
      cum_batches = resume_hdr.cum_batches;
    }

    // FLOPs calculation
    // Forward: W1[H,D] @ x[D,S] = 2*H*D*S, W2[D,H] @ h[H,S] = 2*D*H*S → total
    // fwd = 4*D*H*S Backward dx: W2^T[H,D] @ dy[D,S] = 2*H*D*S, W1^T[D,H] @
    // dh[H,S] = 2*D*H*S → total bwd = 4*D*H*S dW (CPU): dW2[D,H] = dy[D,S] @
    // h^T[S,H] = 2*D*S*H, dW1 same → total dW = 4*D*H*S ANE FLOPs per step =
    // 8*D*H*S (fwd + bwd on ANE) CPU FLOPs per step = 4*D*H*S (dW accumulation)
    ...
    if (!resuming) {
      for (int i = 0; i < H * D; i++)
        W1[i] = 0.01f * sinf(i * 1.3f + 0.7f);
      for (int i = 0; i < D * H; i++)
        W2[i] = 0.01f * cosf(i * 0.9f + 1.1f);
      printf("=== ANE Training: Pipeline Parallel + Grad Accumulation ===\n");
      printf("x:[%d,%d] -> W1:[%d,%d] -> ReLU -> W2:[%d,%d] -> y:[%d,%d]\n", S,
             D, H, D, D, H, S, D);
      printf("Accum %d steps per recompile | Pipeline: compile overlaps ANE "
             "eval\n",
             ACCUM_STEPS);
    ...
    while (step < total_steps) {
      ...
      if (g_compile_count + KERNELS_PER_STEP > MAX_COMPILES) {
        ...
        free_kern(k2_bwd);
        if (!save_checkpoint(
                CKPT_PATH, step, last_loss, D, H, S, total_steps, lr, W1, W2,
                cum_compile_ms + total_compile_ms,
                cum_train_ms + total_train_ms,
                cum_wall_ms +
                    tb_to_ms(mach_absolute_time() - t_wall_start, g_tb),
                cum_steps + total_steps_done, cum_batches + total_batches)) {
          fprintf(stderr,
                  "Warning: checkpoint save failed before exec restart\n");
        }
        double wall = tb_to_ms(mach_absolute_time() - t_wall_start, g_tb);
        printf("[exec() restart at step %d, %d compiles, loss=%.6f, "
               "wall=%.0fms]\n",
      ...
      // But for pipeline, we compile AHEAD: while running batch N, compile for
      // N+1 So we need to update weights BEFORE launching background compile

      uint64_t t_batch = mach_absolute_time();
      for (int a = 0; a < ACCUM_STEPS && step < total_steps; a++, step++) {
        ane_eval_k(k1_fwd, x, h, D, H, S);
        for (int i = 0; i < S * H; i++)
          h_relu[i] = h[i] > 0 ? h[i] : 0;
      ...
      double ane_gflops =
          (ane_flops_per_step * steps_this_batch) / (batch_ms * 1e6);
      double total_gflops =
          (total_flops_per_step * steps_this_batch) / (batch_ms * 1e6);

      if (total_batches % 5 == 1 || total_batches <= 2 || step >= total_steps) {
        printf("step %-5d loss=%-10.6f  %5.1fms/step  ANE=%.2f GFLOPS  "
               "total=%.2f GFLOPS  compiles=%d\n",
               step - steps_this_batch, last_loss, step_ms, ane_gflops,
               total_gflops, g_compile_count);
      }
      ...
      bool can_pipeline = (step < total_steps) &&
                          (g_compile_count + KERNELS_PER_STEP <= MAX_COMPILES);
      ...
      if (can_pipeline) {
        ...
        dispatch_async(g_compile_queue, ^{
          @autoreleasepool {
            uint64_t t0 = mach_absolute_time();
            pc->k1_fwd = compile_kern_with_blob(
                build_blob(pc->W1, pc->H, pc->D), pc->D, pc->H, pc->S);
            pc->k2_fwd = compile_kern_with_blob(
                build_blob(pc->W2, pc->D, pc->H), pc->H, pc->D, pc->S);
            pc->k2_bwd = compile_kern_with_blob(
                build_blob_transposed(pc->W2, pc->D, pc->H), pc->D, pc->H,
                pc->S);
            pc->k1_bwd = compile_kern_with_blob(
                build_blob_transposed(pc->W1, pc->H, pc->D), pc->H, pc->D,
                pc->S);
        ...
        // === While compile runs in background, do ANOTHER batch with OLD
        // kernels ===
        if (step < total_steps && k1_fwd && k2_fwd && k1_bwd && k2_bwd) {
          ...
          // Apply these gradients with reduced LR (stale weights — 1 batch
          // behind)
          ...
          if (total_batches % 5 == 1) {
            ...
            printf("step %-5d loss=%-10.6f  %5.1fms/step  (overlapped with "
                   "compile)  compiles=%d\n",
        ...
        k2_fwd = compile_kern_with_blob(build_blob(W2, D, H), H, D, S);
        k2_bwd =
            compile_kern_with_blob(build_blob_transposed(W2, D, H), D, H, S);
        k1_bwd =
            compile_kern_with_blob(build_blob_transposed(W1, H, D), H, D, S);
        double cms = tb_to_ms(mach_absolute_time() - t0, g_tb);
        total_compile_ms += cms;
        if (!k1_fwd || !k2_fwd || !k1_bwd || !k2_bwd) {
          if (!save_checkpoint(
    ...
    printf("Overhead:        %.0f ms (%.1f%%)\n",
           ...
           100.0 * (total_wall_ms - total_compile_ms - total_train_ms) /
               total_wall_ms);
    printf("\n");
    ...
    if (total_hidden_compile_ms > 0) {
      printf("Hidden compile:  %.0f ms (%.1f%% of total compile)\n",
             total_hidden_compile_ms,
             100.0 * total_hidden_compile_ms / total_compile_ms);
    }
    printf("\n");
    ...
    printf("Weight params:   %d (%.1f KB FP16)\n", H * D + D * H,
           weight_bytes / 1024.0);
    printf("Compile amortization: %.1f ms compile / %d steps = %.2f ms/step "
           "overhead\n",
