// ARCHIVED LEGACY FILE (2026-03-02):
// Superseded by packages/ane-train/src/tiny/tiny_train.m.
// Intentionally kept for historical reference and not wired into
// packages/ane-train/Makefile build targets. tiny_train.m — Train a 2-layer
// linear model on ANE (forward AND backward) y = W2 @ relu(W1 @ x), MSE loss,
// SGD update Forward: ANE conv with baked weights Backward dx: ANE conv with
// transposed baked weights Backward dW: CPU (outer product, memory-bound)
...
static Kern *compile_kern_with_blob(NSData *blob, int in_ch, int out_ch,
  ...
  id desc = ((id (*)(Class, SEL, id, id, id))objc_msgSend)(
      g_D, @selector(modelWithMILText:weights:optionsPlist:), milData, wd, nil);
  if (!desc)
    return NULL;
...
static void ane_eval(Kern *k, const float *in, float *out, int in_ch,
                     int out_ch, int sp) {
  ...
  ((BOOL (*)(id, SEL, unsigned int, id, id, NSError **))objc_msgSend)(
      k->model, @selector(evaluateWithQoS:options:request:error:), 21, @{},
      k->request, &e);
...
int main(int argc, char *argv[]) {
  @autoreleasepool {
    ...
    printf("x:[%d,%d] → W1:[%d,%d] → ReLU → W2:[%d,%d] → y:[%d,%d]\n", S, D, H,
           D, D, H, S, D);
    printf(
        "Forward: ANE conv | Backward dx: ANE conv(W^T) | Backward dW: CPU\n");
    ...
    float *dx_layer = (float *)malloc(
        S * D * sizeof(float)); // not used for update but proves backward works
    ...
    Kern *k1_fwd = NULL,
         *k2_fwd = NULL; // W1: [H,D]→conv(D→H), W2: [D,H]→conv(H→D)
    Kern *k1_bwd = NULL,
         *k2_bwd = NULL; // W1^T: [D,H]→conv(H→D), W2^T: [H,D]→conv(D→H)
    bool on_ane = true;
    ...
    for (int step = 0; step < steps; step++) {
      ...
      if (on_ane && step % recompile_every == 0) {
        ...
        @autoreleasepool {
          ...
          // W2^T: [H,D] as conv weight, input dy [1,D,1,S] → output dh
          // [1,H,1,S]
          k2_bwd =
              compile_kern_with_blob(build_blob_transposed(W2, D, H), D, H, S);
          // W1^T: [D,H] as conv weight, input dh [1,H,1,S] → output dx
          // [1,D,1,S]
          k1_bwd =
