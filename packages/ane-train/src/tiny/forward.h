// forward.h — Forward pass: ANE baked-weight conv for linears, CPU for
// element-wise
#pragma once
#include "model.h"
#include <math.h>
...
// ANE conv eval: input [S, in_dim] row-major → transpose to [in_dim, S]
// channels-first ANE computes conv(W, x) with baked W → output [out_dim, S]
// Transpose back to [S, out_dim] row-major
static void ane_conv_eval(ANEKernel *kernel, const float *x, float *y, int S,
...
// CPU matmul fallback: y = W @ x, W[out_dim, in_dim], x[S, in_dim] → y[S,
// out_dim]
static void cpu_matmul(const float *W, const float *x, float *y, int S,
                       int in_dim, int out_dim) {
  for (int t = 0; t < S; t++)
    for (int i = 0; i < out_dim; i++) {
      float sum = 0;
...
static void cpu_rmsnorm(float *out, const float *x, const float *w, int S,
                        int D) {
  for (int t = 0; t < S; t++) {
    float ss = 0;
    for (int i = 0; i < D; i++)
...
static void cpu_attention(float *out, const float *q, const float *k,
                          const float *v, int S, int n_heads, int head_dim) {
...
static float model_forward(Model *m, const int *tokens, bool use_ane) {
  ...
  for (int l = 0; l < N_LAYERS; l++) {
    ...
    cpu_attention(m->act_attn_out[l], m->act_q[l], m->act_k[l], m->act_v[l], S,
                  nh, hdim);
    ...
    for (int t = 0; t < S; t++)
      for (int i = 0; i < hd; i++)
        m->act_silu[l][t * hd + i] =
            silu_f(m->act_h1[l][t * hd + i]) * m->act_h3[l][t * hd + i];
