...
static bool token_id_in_vocab(uint16_t tok, int vocab, int pos,
                              const char *label) {
  if ((unsigned int)tok >= (unsigned int)vocab) {
    fprintf(stderr, "ERROR: %s token out of range at pos %d: %u (vocab=%d)\n",
            label, pos, (unsigned int)tok, vocab);
    return false;
  }
  return true;
}
...
static void rmsnorm_bwd(float *dx, float *dw, const float *dy, const float *x,
  ...
  for (int i = 0; i < d; i++) {
    ...
    vDSP_vmul(g_rms_tmp, 1, rrms, 1, g_rms_tmp, 1, (vDSP_Length)S);
    float s;
    vDSP_sve(g_rms_tmp, 1, &s, (vDSP_Length)S);
    dw[i] += s;
  }
  free(ss);
  free(rrms);
...
static void adam_update(float *w, const float *g, AdamState *s, int t, float lr,
...
// Data is column-major [V, S], but we process per-column (stride=1 within col
// is v*S+t, stride between v's is S) For vDSP: transpose to row-major scratch
// [S, V] to vectorize softmax per position
static bool cross_entropy_loss_checked(float *loss_out, float *dlogits,
                                       const float *logits,
                                       const uint16_t *targets, int V, int S) {
  // Work in transposed layout [S, V] where each row is one position's logits
  // (contiguous)
  float *buf = (float *)malloc((size_t)S * (size_t)V * 4);
  if (!buf) {
    fprintf(stderr, "ERROR: cross_entropy_loss_checked failed to allocate "
                    "softmax buffer\n");
    return false;
  }
  ...
  for (int t = 0; t < S; t++) {
    ...
    // loss
    int tgt = targets[t];
    if (!token_id_in_vocab((uint16_t)tgt, V, t, "target")) {
      free(buf);
      return false;
    }
  ...
  free(buf);
  *loss_out = total_loss / S;
  return true;
}

static float __attribute__((unused)) cross_entropy_loss(float *dlogits,
                                                        const float *logits,
                                                        const uint16_t *targets,
                                                        int V, int S) {
  float loss = INFINITY;
  (void)cross_entropy_loss_checked(&loss, dlogits, logits, targets, V, S);
  return loss;
}

// Embedding lookup: token_ids → x [DIM, SEQ] (channel-first)
// embed is [VOCAB, DIM] row-major (vocab_size rows, dim cols)
static bool embed_lookup_checked(float *x, const float *embed,
                                 const uint16_t *tokens, int vocab, int dim,
                                 int seq) {
  for (int t = 0; t < seq; t++) {
    int tok = tokens[t];
    if (!token_id_in_vocab((uint16_t)tok, vocab, t, "input"))
      return false;
    for (int d = 0; d < dim; d++) {
...
static void __attribute__((unused)) embed_lookup(float *x, const float *embed,
                                                 const uint16_t *tokens,
                                                 int dim, int seq) {
  (void)embed_lookup_checked(x, embed, tokens, VOCAB, dim, seq);
}

// Embedding backward: accumulate dE[tok] += dx[:,t] for each position
static bool embed_backward_checked(float *d_embed, const float *dx,
                                   const uint16_t *tokens, int vocab, int dim,
                                   int seq) {
  for (int t = 0; t < seq; t++) {
    int tok = tokens[t];
...
static void __attribute__((unused)) embed_backward(float *d_embed,
                                                   const float *dx,
                                                   const uint16_t *tokens,
                                                   int dim, int seq) {
  (void)embed_backward_checked(d_embed, dx, tokens, VOCAB, dim, seq);
}
