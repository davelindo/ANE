...
// Plus 1 static (sdpaBwd2 per layer, no weights) = 12 more but those are
// weight-free Actually sdpaBwd2 has no weights, compile once per layer
// Weight-bearing: fwdAttn(1) + fwdFFN(1) + ffnBwd(1) + sdpaBwd1(1) + qkvBwd(1)
// = 5 per layer 5 * 12 = 60 weight-bearing compiles per batch With
// MAX_COMPILES=100, we get 1 batch of ACCUM_STEPS before restart
...
#define TOTAL_PARAMS                                                           \
  (NLAYERS * LAYER_PARAMS + DIM + VOCAB * DIM) // +rms_final+embed
...
typedef struct {
  ...
  void *client;
  void *perfStats;
  uint64_t lastHwExecNS;
  uint64_t totalHwExecNS;
  uint64_t evalCount;
  uint64_t perfSamples;
  int clientEvalEnabled;
  int warnedClientFallback;
} Kern;
typedef struct {
...
// Globals
static Class g_D, g_I, g_AR, g_AIO, g_APS;
...
static void ane_init(void) {
  ...
  g_APS = NSClassFromString(@"_ANEPerformanceStats");
}
...
static LayerWeights layer_weights_alloc(void) {
  LayerWeights w;
  w.Wq = (float *)malloc(WQ_SZ * 4);
  w.Wk = (float *)malloc(WQ_SZ * 4);
  w.Wv = (float *)malloc(WQ_SZ * 4);
  w.Wo = (float *)malloc(WO_SZ * 4);
  w.W1 = (float *)malloc(W1_SZ * 4);
  w.W2 = (float *)malloc(W2_SZ * 4);
  w.W3 = (float *)malloc(W3_SZ * 4);
...
static LayerAdam layer_adam_alloc(void) {
  LayerAdam a;
  a.Wq = adam_alloc(WQ_SZ);
  a.Wk = adam_alloc(WQ_SZ);
  a.Wv = adam_alloc(WQ_SZ);
  a.Wo = adam_alloc(WO_SZ);
  a.W1 = adam_alloc(W1_SZ);
  a.W2 = adam_alloc(W2_SZ);
  a.W3 = adam_alloc(W3_SZ);
  a.rms_att = adam_alloc(DIM);
  a.rms_ffn = adam_alloc(DIM);
  return a;
}
...
static LayerGrads layer_grads_alloc(void) {
  LayerGrads g;
  g.Wq = (float *)calloc(WQ_SZ, 4);
  g.Wk = (float *)calloc(WQ_SZ, 4);
  g.Wv = (float *)calloc(WQ_SZ, 4);
  g.Wo = (float *)calloc(WO_SZ, 4);
  g.W1 = (float *)calloc(W1_SZ, 4);
  g.W2 = (float *)calloc(W2_SZ, 4);
  g.W3 = (float *)calloc(W3_SZ, 4);
  g.rms_att = (float *)calloc(DIM, 4);
  g.rms_ffn = (float *)calloc(DIM, 4);
  return g;
}
static void layer_grads_zero(LayerGrads *g) {
  memset(g->Wq, 0, WQ_SZ * 4);
  memset(g->Wk, 0, WQ_SZ * 4);
  memset(g->Wv, 0, WQ_SZ * 4);
  memset(g->Wo, 0, WO_SZ * 4);
  memset(g->W1, 0, W1_SZ * 4);
  memset(g->W2, 0, W2_SZ * 4);
  memset(g->W3, 0, W3_SZ * 4);
  memset(g->rms_att, 0, DIM * 4);
  memset(g->rms_ffn, 0, DIM * 4);
}
